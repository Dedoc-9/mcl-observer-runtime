...
The system issues deterministic, provenance-bound judgments about agreement among declared observers and none about the world they observe — it locates, reproducibly, where the user's notion of "the system" stops being singular, declares what it did not measure, and names the layer of the stack that owes the verdict it refuses to counterfeit. Every clause is now load-bearing against a built mechanism: deterministic (byte-identical reruns under pinned epoch), provenance-bound (chain hashes, kernel and encoder fingerprints, seed registry), declared observers (encoder registration with hard failure on anything undeclared — and "declared" quietly carries the family caveat: agreement is only as meaningful as the observers are independent), none about the world (claim_class on every output, a schema with no field that can hold a verdict), declares what it did not measure (unmeasured_axes, measured_region_only), and names the layer that owes the verdict (the handoff contract's decline-and-redirect table, which routes every closure request to host-system predicates by name). The verb counterfeit stays — it's the precise word, since a verdict issued from a lossy quotient wouldn't merely be unauthorized, it would be a forgery of a judgment the underlying measurement cannot back.

one sentence on why I didn't define Φ:
Because Φ was intentionally left undefined as a primitive observable of disagreement rather than a constructed quantity, so any formal definition would incorrectly collapse it into the very representational assumptions it is meant to expose.
